		...
		<table class='table table-striped table-hover table-condensed'>
			...
			<tbody>
				...
				<tr class='row-fluid rowlink' data-target='Violation_details/<?=$violation->citation_number;?>' title='Violation'>
					...
					<td>
						<a <? if($totalAmount <= 0){ echo "class='disabled'"; } ?> href="<?=base_url();?>Payment/process_payment/<?=$violation->violation_number;?>" class="btn btn-success btn-xs">PAY NOW</a> | <a href="<?=base_url();?>Information" class="btn btn-primary btn-xs">How to Resolve</a>
					</td>
					<? } ?>
			</tbody>
		</table>   
