...
<?
		...
		else
		{
			...
			$warrantCount = $this->Violations_model->isWarrant($lastName,$SSN);
				...
				?>
				<Response>
					<Message>Hi <?=$resp[0]->first_name;?> <?=$resp[0]->last_name;?> You have <? if($msgC) <?=$msgCount;?> violations and <? if($warrantCount == 1){ echo $warrantCount;?> warrant. <? } else{ echo $warrantCount; ?> warrants. <? } ?>To list all violations please use format (Last Name*Last 4 SSN*All). For more information contact 573-526-2407. </Message>
				</Response>
				<?
/* 					$i++;
				}
