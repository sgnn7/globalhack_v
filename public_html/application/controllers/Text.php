...
<?
		...
		else
		{
			...
			if($msgCount > 0){
				...
				?>
				<Response>
					<Message>Hi <?=$resp[$i]->first_name;?> <?=$resp[$i]->last_name;?>| Citation#:<?=$resp[$i]->citation_number;?>| Amount Owed:$<?=$totalAmount;?>| Court Name:<?=$resp[$i]->court_location;?>| Court Date:<?=$resp[$i]->court_date;?> | Count :<?=$msgCount;?></Message>
				</Response>
				<?
					$i++;
				}
